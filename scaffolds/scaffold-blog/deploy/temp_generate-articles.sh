#!/usr/bin/env bash
# Generate 12 articles with images for each of the 27 blogs
...
# Blog configurations: domain|topic1|topic2|...|topic12
BLOGS=(
"survivalpatriots.com|emergency preparedness|home security tips|survival food storage|first aid essentials|self-defense techniques|water purification|bug-out bag essentials|disaster planning|off-grid living|wilderness survival|emergency communication|financial preparedness"
"healthhubmedia.com|immune system boosting|heart health tips|mental wellness strategies|sleep optimization|nutrition fundamentals|exercise routines|stress management|healthy aging|digestive health|brain health|joint health|eye care"
...
"conservativetips.com|traditional values in health|family wellness traditions|self-reliance health|common sense medicine|cost-effective healthcare|home remedies|natural living|faith and health|community health|personal responsibility|health freedom|informed choices"
"conservativeremedies.com|time-tested remedies|grandmother cures|traditional medicine|folk remedies|heritage healing|old-fashioned treatments|proven solutions|natural alternatives|homemade remedies|kitchen medicine|garden pharmacy|ancestral wisdom"
...
"conservativehealthnews.com|health freedom news|medical industry updates|vaccine developments|pharmaceutical news|healthcare legislation|alternative medicine news|health policy changes|medical research news|treatment breakthroughs|health rights advocacy|wellness industry news|regulatory updates"
"conservativehealthhacks.com|budget health tips|DIY wellness|frugal fitness|homegrown health|self-care savings|natural solutions|common sense wellness|practical health hacks|affordable alternatives|smart health choices|resourceful wellness|value-based health"
...
"myalternativenews.com|unreported health stories|suppressed research|health industry secrets|censored treatments|hidden cures|pharmaceutical cover-ups|natural solution news|wellness whistleblowers|health truth movement|independent research|alternative perspectives|health freedom fighters"
...
echo "================================================================="
echo "Generating 12 articles for each of 27 blogs"
...
for blog_config in "${BLOGS[@]}"; do
  ...
  # Add profile for this blog
  echo "Adding profile: $profile_name -> https://$domain"
  bun run cli/index.ts config profile add "$profile_name" --url "https://$domain" 2>/dev/null || true

  # Login to get API key
  echo "Logging in to $domain..."
  if ! bun run cli/index.ts --profile "$profile_name" config login "$ADMIN_EMAIL" "$ADMIN_PASSWORD" 2>&1; then
    echo "  WARNING: Login failed for $domain, skipping..."
    continue
  fi

  # Generate 12 articles (parts 1-12 are the topics)
  for i in {1..12}; do
    topic="${parts[$i]}"
    if [ -z "$topic" ]; then
      echo "  [$i/12] No topic defined, skipping..."
      continue
    fi

    echo "  [$i/12] Generating article about: $topic"
    ...
    if bun run cli/index.ts --profile "$profile_name" ai generate \
      ...
      echo "    WARNING: Failed to generate article about $topic"
    fi
