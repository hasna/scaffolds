export interface EnvVar {
  key: string;
  label: string;
  description: string;
  default?: string;
  required: boolean;
  secret: boolean;
  group: string;
  hint?: string;
}

export const ENV_VARS: EnvVar[] = [
  // App
  {
    key: "APP_URL",
    label: "Application URL",
    description: "The public URL of your application (used for OAuth callbacks, emails, etc.)",
    default: "http://localhost:3000",
    required: true,
    secret: false,
    group: "App",
  },
  {
    key: "OPENAI_API_KEY",
    label: "OpenAI API Key",
    description: "Optional — enables AI features in your SaaS app",
    required: false,
    secret: true,
    group: "App",
    hint: "Get from https://platform.openai.com/api-keys",
  },

  // Auth
  {
    key: "AUTH_SECRET",
    label: "Auth Secret",
    description: "Random secret used to sign session tokens. Generate with: openssl rand -base64 32",
    required: true,
    secret: true,
    group: "Auth",
    hint: "Run: openssl rand -base64 32",
  },
  {
    key: "AUTH_GOOGLE_ID",
    label: "Google OAuth Client ID",
    description: "Google OAuth 2.0 client ID for sign-in with Google",
    required: false,
    secret: false,
    group: "Auth",
    hint: "Get from https://console.cloud.google.com/apis/credentials",
  },
  {
    key: "AUTH_GOOGLE_SECRET",
    label: "Google OAuth Client Secret",
    description: "Google OAuth 2.0 client secret",
    required: false,
    secret: true,
    group: "Auth",
    hint: "Get from https://console.cloud.google.com/apis/credentials",
  },
  {
    key: "AUTH_GITHUB_ID",
    label: "GitHub OAuth App Client ID",
    description: "GitHub OAuth app client ID for sign-in with GitHub",
    required: false,
    secret: false,
    group: "Auth",
    hint: "Get from https://github.com/settings/developers",
  },
  {
    key: "AUTH_GITHUB_SECRET",
    label: "GitHub OAuth App Client Secret",
    description: "GitHub OAuth app client secret",
    required: false,
    secret: true,
    group: "Auth",
    hint: "Get from https://github.com/settings/developers",
  },
  {
    key: "AUTH_EMAIL_FROM",
    label: "Auth Email From Address",
    description: "The sender address used for magic-link / verification emails",
    default: "noreply@example.com",
    required: false,
    secret: false,
    group: "Auth",
  },

  // Database
  {
    key: "DATABASE_URL",
    label: "PostgreSQL Connection URL",
    description: "Your Postgres DB URL. Start local DB with: docker compose up -d postgres",
    default: "postgresql://postgres:postgres@localhost:5432/scaffold_saas",
    required: true,
    secret: true,
    group: "Database",
    hint: "Format: postgresql://user:password@host:port/database",
  },

  // Email
  {
    key: "RESEND_API_KEY",
    label: "Resend API Key",
    description: "API key for sending transactional emails via Resend",
    required: false,
    secret: true,
    group: "Email",
    hint: "Get from https://resend.com/api-keys",
  },

  // Redis
  {
    key: "REDIS_URL",
    label: "Redis Connection URL",
    description: "Redis URL for BullMQ workers and caching. Start with: docker compose up -d redis",
    default: "redis://localhost:6379",
    required: false,
    secret: false,
    group: "Redis",
    hint: "Format: redis://host:port or redis://:password@host:port",
  },
];
